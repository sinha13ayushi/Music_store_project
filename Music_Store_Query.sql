...
select* from invoice;
SELECT 
    billing_city, ROUND(total) AS InvoiceTotal
FROM
    invoice
...
SELECT 
    billing_city, ROUND(SUM(total), 2) AS InvoiceTotal
FROM
    invoice
GROUP BY billing_city
ORDER BY InvoiceTotal DESC
...
FROM
    ...
    invoice_line AS il ON il.invoice_id = i.invoice_id
        JOIN
    track AS tr ON tr.track_id = il.track_id
        JOIN
    album AS alb ON alb.album_id = tr.album_id
        JOIN
